<script type="text/javascript">
  ...
  window.location.href = 'index.php?page=loan-bank';
}
</script>
...
<?php
	//MODALS
	include $_SERVER['DOCUMENT_ROOT']."/catafa/includes/loanable-balance-deposit.php";
	include $_SERVER['DOCUMENT_ROOT']."/catafa/includes/loanable-balance-withdraw.php";



		// BANK BALANCE
	$sql = "SELECT * FROM loan_bank WHERE id = 1";
	$query = mysqli_query($conn, $sql);
	$row = mysqli_fetch_assoc($query);
	$loan_balance_amount = $row['amount'];
	$loan_balance_last_update = $row['last_updated'];

 ?>
<div class="container-fluid p-4 bg-light" style="width: 100%; height: 90vh; overflow-y: scroll; overflow-x: hidden;">
	<div class="col-12 p-3 shadow bg-white rounded">
		<div class="d-flex flex-row mb-4 align-items-center">
			<div class="mx-2 d-flex align-items-center">
				<h3 style="color: #333333;">Funds</h3>
			</div>
		</div>
		<div class="row">
			<div class="col-5">
				<div class="mx-1 p-2 data_table bg-white rounded " style="z-index: 1;">
				  ...
				  <div class="row">
				    <h1 class="display-5 text-center">₱<?php echo number_format($loan_balance_amount, 2, '.', ','); ?></h1>
				    <p class="text-secondary text-center">Last Updated: <?php echo date_format( date_create($loan_balance_last_update), 'F jS, Y'); ?></p>
				  </div>
				  <div class="d-flex justify-content-end">
					<button class="btn btn-primary my-0" data-bs-toggle="modal" data-bs-target="#loanable-balance-deposit-modal">Deposit</button>
					<button class="mx-1 btn btn-danger my-0" data-bs-toggle="modal" data-bs-target="#loanable-balance-withdraw-modal">Withdraw</button>
				  </div>
				</div>
			</div>
			<div class="col mx-1">
				<div class="col">
					<div class="data_table">
			            <div class="row mb-1 w-100">
		                		<div class="col">
		                			<h4 class="mb-2" style="color:#333333;">Wallet History</h4>
		                		</div>
			                	<div class="col dropdown d-flex justify-content-end">
								 	<?php include $_SERVER['DOCUMENT_ROOT']."/catafa/includes/date-range-nav.php"; ?>
								</div>
							</div>
						<table id="loan-wallet-history" class="table table-hover table-bordered">
						  <thead>
						    <tr>
						     	<th>#</th>
						      	<th>Amount</th>
						  ...
						  <tbody>
						  	<?php
								...
								$sql = "SELECT * FROM loan_bank_history WHERE date BETWEEN '$start_date' AND '$end_date' ORDER BY date desc";
								$query = mysqli_query($conn, $sql);
						    ...
						    <tr>
						    	...
						    	<td class="align-middle"><?php echo date("F j, Y", strtotime($date)); ?></td>
