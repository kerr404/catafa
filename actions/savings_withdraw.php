<?php 
	...
	if (isset($_POST['balanceCheck'])) {
		...
		$balance = $check_row['account_balance'];
	...
	if (isset($_POST['account-number'])) {
		...
		$amount_withdraw = $_POST['withdraw-amount'];
		...
		$sql0 = "SELECT * FROM savings_withdraw WHERE ref_no='$withdraw_ref_num';";
		...
		if (mysqli_num_rows($query) == 0) {
			...
			$sql2 = "INSERT INTO savings_withdraw (ref_no, account_number, account_holder, withdraw_amount, withdraw_date) VALUES ('$withdraw_ref_num', '$account_number', '$account_holder', '$amount_withdraw', '$date')";
			$query2 = mysqli_query($conn, $sql2);

			// Check the query is successful. If yes, then update the current savings balance.
			if($query2){
				$sql3 = "UPDATE savings_account_list SET account_balance = (account_balance - '$amount_withdraw') WHERE account_number = '$account_number'";
				$query3 = mysqli_query($conn, $sql3);

				if ($query3) {
						$sql4 = "INSERT INTO savings_history (member_id, ref_no, account_number, credit, description, date) VALUES ('$account_holder', '$withdraw_ref_num', '$account_number', '$amount_withdraw', 'Balance Withdraw', '$date')";
						$query4 = mysqli_query($conn, $sql4);

							// Check if the balance is successfully credited to account balane. If not, then undo the queries made.
							if($query4){
								$sql5 = "UPDATE savings_bank SET amount = (amount - '$amount_withdraw'), last_updated = '$date'";
